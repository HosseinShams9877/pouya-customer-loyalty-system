  ...
  try {
    ...
    if (!type || !description) {
      return res.status(400).json({
        success: false,
        message: 'نوع و توضیحات تعامل الزامی است',
      });
    }
    ...
    if (!['CALL', 'MEETING', 'MESSAGE'].includes(type)) {
      return res.status(400).json({
        success: false,
        message: 'نوع تعامل نامعتبر است. مقادیر مجاز: CALL, MEETING, MESSAGE',
      });
    }
    ...
    // SALES_REP can only add interactions to their own leads
    ...
    if (type === 'CALL') {
      ...
      if (!customer) {
        const baseTier = await prisma.loyaltyTier.findFirst({ where: { isActive: true, audienceType: { in: ['CONTRACTOR', 'ALL'] } }, orderBy: { minPoints: 'asc' } });
        customer = await prisma.customer.create({
          data: { fullName: lead.fullName, mobile: lead.mobile, company: lead.company, customerType: 'CONTRACTOR', assignedToId: lead.assignedToId || req.user.id, tierId: baseTier?.id || null },
        });
      } else if (!customer.assignedToId && (lead.assignedToId || req.user.id)) {
        customer = await prisma.customer.update({ where: { id: customer.id }, data: { assignedToId: lead.assignedToId || req.user.id } });
      }
      csat = await csatService.createCsatToken({ interactionId: interaction.id, customerId: customer.id, customerName: customer.fullName, customerMobile: customer.mobile, leadId: lead.id, assignedToId: lead.assignedToId || req.user.id }).catch((error) => ({ error: error.message }));
    }

    res.status(201).json({ success: true, message: type === 'CALL' ? 'تماس ثبت و نظرسنجی رضایت ارسال شد' : 'تعامل ثبت شد', data: { ...interaction, csat } });
  } catch (err) {
...
router.get('/upcoming', async (req, res) => {
  try {
    ...
    // Get lead IDs for current user (or all for ADMIN)
    const leadWhere = {};
