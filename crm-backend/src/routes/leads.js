 ...
 */
     ...
     const [leads, total] = await Promise.all([
       prisma.lead.findMany({
         ...
         include: {
           ...
           customer: {
             select: { id: true, fullName: true, mobile: true, status: true },
           },
           _count: {
 ...
 router.delete('/:id', async (req, res) => {
   try {
     ...
     // SALES_REP can only delete their own leads
     if (req.user.role === 'SALES_REP' && existing.assignedToId && existing.assignedToId !== req.user.id) {
       return res.status(403).json({
         success: false,
         message: 'شما فقط می‌توانید سرنخ‌های خود را حذف کنید',
       });
     }
 ...
 // If stage=WON, auto-convert to Customer
 // ════════════════════════════════════════════
 router.patch('/:id/stage', async (req, res) => {
   try {
     ...
     if (stage === 'WON' && existing.stage !== 'WON') {
       // 1. چک کن مشتری با این موبایل قبلاً وجود داره یا نه
     ...
     // اگر مشتری ساخته شد، customerId رو به Lead اضافه کن
     if (customer) {
