...
function LeadList({ onSelect }) {
  ...
  const filtered = leads.filter(l =>
    !search || l.fullName?.includes(search) || l.mobile?.includes(search) || l.company?.includes(search)
  );

  // شمارش سرنخ‌ها در هر مرحله
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <Card className="p-4">
          ...
          <div className="flex bg-slate-100 dark:bg-slate-800 rounded-lg p-1">
            <button
              ...
              className={cn(
                ...
                view === 'list' ? 'bg-white dark:bg-surface-700 text-brand-600 dark:text-brand-400 shadow-sm' : 'text-slate-500 dark:text-slate-400'
              )}
            >
            ...
            <button
              ...
              className={cn(
                'px-3 py-1.5 rounded-md text-xs font-medium transition-all flex items-center gap-1.5',
                view === 'kanban' ? 'bg-white dark:bg-surface-700 text-brand-600 dark:text-brand-400 shadow-sm' : 'text-slate-500 dark:text-slate-400'
              )}
            >
        ...
        {/* Stage filter chips با تعداد */}
        <div className="flex gap-2 flex-wrap mt-3">
          <button
            onClick={() => setStageFilter('')}
            className={cn(
              'px-3 py-1.5 rounded-lg text-xs font-medium transition-all flex items-center gap-1.5',
              stageFilter === '' ? 'bg-brand-600 text-white shadow-sm' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700'
            )}
          >
            همه
            <span className={cn(
          ...
          {STAGES.map(s => {
            ...
            return (
              <button
                ...
                className={cn(
                  'px-3 py-1.5 rounded-lg text-xs font-medium transition-all flex items-center gap-1.5',
                  stageFilter === s ? 'bg-brand-600 text-white shadow-sm' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700'
                )}
              >
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          action={<Button onClick={() => setModalOpen(true)} icon={Plus} variant="outline" size="sm">ثبت سرنخ جدید</Button>}
        />
      ) : view === 'kanban' ? (
            ...
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-xs text-slate-500 dark:text-slate-400 truncate" dir="ltr">{lead.company || lead.mobile}</div>
                </div>
              </div>
              <div className="flex items-center gap-3 flex-shrink-0">
                {lead.estimatedValue > 0 && (
                  <div className="hidden sm:block text-left">
                    <div className="text-xs font-mono text-slate-700 dark:text-slate-200">{Number(lead.estimatedValue).toLocaleString('fa-IR')}</div>
      ...
      {columns.map(col => (
        ...
        >
          <div className="p-3 border-b border-slate-100 dark:border-slate-800">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold text-slate-700 dark:text-slate-200">{col.cfg.label}</h3>
              </div>
          ...
          <div className="flex-1 p-2 space-y-2 overflow-y-auto max-h-[600px]">
            {col.items.length === 0 ? (
              <div className="text-center py-8 text-xs text-slate-300 dark:text-slate-600">خالی</div>
            ) : col.items.map(lead => {
              ...
              return (
                ...
                >
                  ...
                  <div className="text-xs text-slate-500 dark:text-slate-400 truncate mt-0.5" dir="ltr">{lead.company || lead.mobile}</div>
                  {lead.estimatedValue > 0 && (
                    <div className="mt-2 pt-2 border-t border-slate-100 dark:border-slate-700 flex items-center justify-between">
                      <span className="text-[10px] text-slate-400">{formatRelative(lead.createdAt)}</span>
...
function LeadDetail({ leadId, onBack }) {
  ...
  const fetchLead = useCallback(async () => {
    ...
    try {
      const [lRes, iRes] = await Promise.all([
        ...
        interactionService.listByLead(leadId, { pageSize: 50 }),
      ]);
      setLead(lRes?.data || lRes);
  ...
  const handleStageChange = async (newStage) => {
    if (newStage === 'LOST') {
      setPendingStage(newStage); setLostModal(true); return;
    }
    setStageLoading(true);
    try {
      await leadService.updateStage(leadId, { stage: newStage });
      fetchLead();
    } catch {} finally { setStageLoading(false); }
  };
  ...
  const handleAddInteraction = async (e) => {
    ...
    setIntLoading(true);
    try {
      await interactionService.create(leadId, { type: intType, description: intDesc, nextFollowUpDate: intDate || null });
      setIntDesc(''); setIntDate(''); setShowForm(false);
      fetchLead();
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-center gap-3">
        <button onClick={onBack} className="p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-300">
          <ArrowRight className="w-5 h-5" />
        </button>
        <div>
          <h1 className="text-xl font-bold text-slate-900 dark:text-white">{lead.fullName}</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400" dir="ltr">{lead.company || lead.mobile}</p>
        </div>
      </div>
      ...
      <Card padded>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <InfoItem icon={MapPin} label="پروژه" value={lead.projectName || '—'} />
          <InfoItem icon={DollarSign} label="ارزش تخمینی" value={lead.estimatedValue ? `${Number(lead.estimatedValue).toLocaleString('fa-IR')} ریال` : '—'} />
        </div>
        {lead.description && (
          <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-700">
            <p className="text-sm text-slate-600 dark:text-slate-300">{lead.description}</p>
          </div>
        )}
      </Card>

      {/* Stage Changer — بهبود یافته با progress */}
      <Card padded>
        <h3 className="text-sm font-bold text-slate-700 dark:text-slate-200 mb-3">تغییر مرحله قیف فروش</h3>
        <div className="flex flex-wrap gap-2">
          {STAGES.map((stage, idx) => {
            const cfg = leadStageConfig[stage];
            const isCurrent = stage === lead.stage;
            const isPast = idx < currentIdx;
            const isDisabled = isCurrent || lead.stage === 'WON' || (lead.stage === 'LOST' && stage !== 'PROFORMA') || isPast || stageLoading;
            return (
              <button
                key={stage}
                disabled={isDisabled}
                ...
                className={cn(
                  ...
                  isCurrent ? cn('ring-2 ring-brand-500', cfg.color) :
                  isDisabled ? 'opacity-40 cursor-not-allowed bg-slate-50 dark:bg-slate-800 text-slate-400' :
      ...
      {lead.stage === 'LOST' && (
        <div className="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm font-bold text-red-700 dark:text-red-300">این سرنخ باخته شده</span>
          </div>
          <div className="text-sm text-red-600 dark:text-red-400">
            <div>دلیل: {lead.lostReason === 'PRICE' ? 'قیمت بالا' : lead.lostReason === 'COMPETITOR' ? 'خرید از رقیب' : lead.lostReason === 'PAYMENT_TERMS' ? 'شرایط پرداخت' : lead.lostReason === 'MARKET_DROP' ? 'کاهش ساخت‌وساز' : 'سایر'}</div>
            {lead.competitorPrice && <div className="mt-1">قیمت رقیب: {Number(lead.competitorPrice).toLocaleString('fa-IR')} ریال</div>}
          </div>
        </div>
      )}
      ...
      <Card padded>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-bold text-slate-700 dark:text-slate-200">تاریخچه تعاملات</h3>
          <button onClick={() => setShowForm(!showForm)} className="text-xs text-brand-600 hover:text-brand-700 dark:text-brand-400 font-medium">
            {showForm ? 'انصراف' : '+ ثبت تعامل جدید'}
          </button>
        </div>

        {showForm && (
          <form onSubmit={handleAddInteraction} className="mb-4 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700 animate-fade-in">
              ...
              {INTERACTION_TYPES.map(t => (
                <button key={t.value} type="button" onClick={() => setIntType(t.value)}
                  className={cn('inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors',
                    intType === t.value ? 'bg-brand-600 text-white' : 'bg-white dark:bg-surface-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700')}>
                  <t.icon className="w-3.5 h-3.5" />{t.label}
                </button>
              ))}
            </div>
            <textarea value={intDesc} onChange={e => setIntDesc(e.target.value)} placeholder="نتیجه تماس یا توضیحات..." rows={3} required
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="flex-1">
                <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">تاریخ پیگیری بعدی</label>
                <input type="date" value={intDate} onChange={e => setIntDate(e.target.value)}
                  className="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-surface-800 text-sm text-slate-900 dark:text-white" dir="ltr" />
              </div>
              <div className="flex items-end">
                <button type="submit" disabled={intLoading || !intDesc.trim()}
                  className={cn('px-6 py-2.5 rounded-lg text-sm font-medium text-white transition-colors',
                    intDesc.trim() ? 'bg-brand-600 hover:bg-brand-700' : 'bg-slate-200 dark:bg-slate-700 text-slate-400 cursor-not-allowed')}>
                  <Send className="w-4 h-4 inline ml-1" />{intLoading ? 'در حال ثبت...' : 'ثبت'}
                </button>
              </div>
            </div>
          </form>
        ...
        {interactions.length === 0 ? (
          <EmptyState title="تعاملی ثبت نشد" description="اولین تعامل خود را ثبت کنید" />
        ) : (
          <div className="relative pr-6 space-y-4">
            ...
            {interactions.map((int, idx) => {
              const typeIcon = int.type === 'CALL' ? Phone : int.type === 'MEETING' ? User : MessageSquare;
                ...
                <div key={int.id || idx} className="relative animate-fade-in">
                  ...
                  <div className="bg-white dark:bg-surface-800 p-3 rounded-lg border border-slate-100 dark:border-slate-800">
                    <div className="flex items-center justify-between mb-1">
                      <Badge color={int.type === 'CALL' ? 'bg-blue-100 text-blue-700' : int.type === 'MEETING' ? 'bg-purple-100 text-purple-700' : 'bg-amber-100 text-amber-700'}>
                        {int.type === 'CALL' ? 'تماس' : int.type === 'MEETING' ? 'جلسه' : 'پیام'}
                      </Badge>
                      <span className="text-xs text-slate-400 dark:text-slate-500">{formatDateTime(int.createdAt)}</span>
                    </div>
                    <p className="text-sm text-slate-700 dark:text-slate-200">{int.description}</p>
                    {int.nextFollowUpDate && (
                      <div className="flex items-center gap-1 mt-2 text-xs text-slate-500 dark:text-slate-400">
                        <Calendar className="w-3 h-3" />پیگیری بعدی: {formatDate(int.nextFollowUpDate)}
                      </div>
                    )}
                  </div>
                </div>
      ...
      <LostReasonModal open={lostModal} onClose={() => setLostModal(false)} onSubmit={handleLostSubmit} />
    </div>
  );
}
  ...
  return (
    <div className="flex items-start gap-2">
      ...
      <div>
        ...
        <div className="text-sm font-medium text-slate-900 dark:text-white" dir="ltr">{value}</div>
      </div>
    </div>
  );
}
